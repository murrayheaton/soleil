...
from ..models.sync_state import SyncOperation, SyncStatus, GoogleService
from app.models.user import Band
from modules.drive.services.drive_client import GoogleDriveService
from app.services.google_sheets import GoogleSheetsService
from modules.content.services.content_parser import parse_filename
...
class SyncEngine:
    ...
    async def stop(self) -> None:
        ...
        while self._active_syncs and wait_time < 30:
            logger.info(
                f"Waiting for {len(self._active_syncs)} active syncs to complete..."
            )
        ...
        if self._active_syncs:
            logger.warning(
                f"Forcibly stopping with {len(self._active_syncs)} active syncs"
            )
    ...
    async def handle_webhook(self, webhook_data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.debug(
                f"Webhook event queued: {event.event_type} for {event.resource_id}"
            )
    ...
    def register_event_handler(
        self, event_type: SyncEventType, handler: Callable[[SyncEvent], Any]
    ) -> None:
    ...
    async def _process_sync_queue(self) -> None:
        ...
        while self._running:
            try:
                ...
                try:
                    event = await asyncio.wait_for(self._sync_queue.get(), timeout=1.0)
    ...
    async def _process_sync_event(self, event: SyncEvent) -> None:
        ...
        try:
            logger.debug(
                f"Processing sync event: {event.event_type} for {event.resource_id}"
            )
    ...
    @backoff.on_exception(backoff.expo, Exception, max_tries=3, max_time=300)
    async def _handle_full_sync(self, event: SyncEvent, session: AsyncSession) -> None:
        ...
        if band.google_drive_folder_id:
            await self._sync_drive_folder(
                drive_service, band.google_drive_folder_id, band_id, session
            )

        # Sync Sheets data
        if band.google_sheets_id:
            await self._sync_sheets_data(
                sheets_service, band.google_sheets_id, band_id, session
            )
    ...
    async def _handle_file_change(
        self, event: SyncEvent, session: AsyncSession
    ) -> None:
    ...
    async def _handle_sheet_change(
        self, event: SyncEvent, session: AsyncSession
    ) -> None:
    ...
    async def _process_file_batch(
        self, files: List[Dict[str, Any]], band_id: int, session: AsyncSession
    ) -> None:
        ...
        try:
            ...
            if "resourceId" in webhook_data:
                ...
                event_type = event_type_map.get(
                    resource_state, SyncEventType.FILE_UPDATED
                )
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            **self.stats,
            ...
            "last_sync_times": {
                band_id: ts.isoformat() for band_id, ts in self._last_sync_times.items()
            },
        }
