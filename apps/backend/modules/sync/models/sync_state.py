...
class SyncOperation(Base):
    ...
    google_webhook_id = Column(
        String(255), nullable=True
    )  # For webhook-triggered syncs
    ...
    sync_items = relationship(
        "SyncItem", back_populates="sync_operation", cascade="all, delete-orphan"
    )
...
class SyncItem(Base):
    ...
    sync_operation_id = Column(
        Integer, ForeignKey("sync_operations.id"), nullable=False, index=True
    )
...
class WebhookEvent(Base):
    ...
    sync_operation_id = Column(
        Integer, ForeignKey("sync_operations.id"), nullable=True, index=True
    )
...
class SyncConfiguration(Base):
    ...
    # Band Association
    band_id = Column(
        Integer, ForeignKey("bands.id"), nullable=False, unique=True, index=True
    )
    ...
    # File Filtering
    file_extensions_allowed = Column(
        JSON, default=lambda: [".pdf", ".mp3", ".wav", ".m4a"]
    )
