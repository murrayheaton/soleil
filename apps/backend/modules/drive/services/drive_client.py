...
class GoogleDriveService(StatsMixin):
        ...
        async with self._get_service() as service:
            while True:
                ...
                try:
                    ...
                    logger.debug(
                        f"Retrieved {len(batch_files)} files, total: {len(files)}"
                    )
    ...
    async def batch_get_metadata(
        self, file_ids: List[str], fields: str = "*"
    ) -> List[Dict[str, Any]]:
        """
            ...
            for i in range(0, len(file_ids), batch_size):
                ...
                def make_batch_request():
                    ...
                    def callback(request_id, response, exception):
                        if exception:
                            logger.error(
                                f"Error in batch request {request_id}: {exception}"
                            )
                ...
                for request_id, response in batch_response.items():
                    if "error" not in response:
                        ...
                        logger.warning(
                            f"Failed to get metadata for file in batch: {response['error']}"
                        )
    ...
    async def setup_webhook(
        self, folder_id: str, webhook_url: str, webhook_secret: Optional[str] = None
    ) -> Dict[str, str]:
        """
        ...
        async with self._get_service() as service:
            ...
            def make_request():
                return (
                    service.files()
                    .watch(fileId=folder_id, body=channel_config)
                    .execute()
                )
    ...
    async def stop_webhook(self, channel_id: str, resource_id: str) -> None:
        ...
        async with self._get_service() as service:
            ...
            def make_request():
                return (
                    service.channels()
                    .stop(body={"id": channel_id, "resourceId": resource_id})
                    .execute()
                )
    ...
    async def process_files_for_sync(
        self, folder_id: str, since: Optional[datetime] = None
    ) -> List[Dict[str, Any]]:
        """
