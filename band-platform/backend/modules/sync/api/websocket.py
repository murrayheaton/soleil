...
class ConnectionManager:
    ...
    async def connect(
        self, websocket: WebSocket, client_id: str, user_id: int, band_id: int
    ):
        ...
        logger.info(
            f"WebSocket client {client_id} connected for user {user_id} in band {band_id}"
        )
...
async def handle_websocket_message(
    message: dict, client_id: str, user_id: int, band_id: int
):
    """
    ...
    if message_type == "ping":
        ...
        await manager.send_personal_message(
            {"type": "pong", "timestamp": "2024-07-23T17:00:00Z"}, client_id
        )
        ...
        await manager.send_personal_message(
            {"type": "stats", "data": stats, "timestamp": "2024-07-23T17:00:00Z"},
            client_id,
        )
