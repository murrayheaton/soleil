...
from modules.drive.services.drive_client import GoogleDriveService
from app.services.folder_organizer import (
    FolderOrganizer,
    create_user_folder_if_needed,
    estimate_sync_time,
)
...
class FileSynchronizer:
    ...
    def __init__(
        ...
        self.organizer = organizer or FolderOrganizer(
            credentials=drive_service.credentials, db_session=db_session
        )
    ...
    async def sync_source_to_user_folders(
        ...
        try:
            ...
            source_files = await self.drive_service.process_files_for_sync(
                source_folder_id, since=since_time
            )

            logger.info(
                f"Found {len(source_files)} files to sync (since: {since_time})"
            )
            ...
            for user in users:
                try:
                    ...
                    sync_results["total_shortcuts_created"] += user_result.get(
                        "shortcuts_created", 0
                    )
                    sync_results["total_shortcuts_deleted"] += user_result.get(
                        "shortcuts_deleted", 0
                    )
            ...
            self.sync_stats["shortcuts_created"] += sync_results[
                "total_shortcuts_created"
            ]
            self.sync_stats["shortcuts_deleted"] += sync_results[
                "total_shortcuts_deleted"
            ]
            ...
            logger.info(
                f"Sync completed: {sync_results['users_processed']} users, "
    ...
    async def sync_single_user(
        self, user_id: int, source_folder_id: str, force_reorganize: bool = False
    ) -> Dict[str, Any]:
        """
    ...
    async def detect_file_changes(self, webhook_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            logger.info(
                f"Processing webhook notification: {webhook_data.get('resourceState', 'unknown')}"
            )
            ...
            if resource_state not in ["update", "add", "remove"]:
                ...
                return {
                    "status": "ignored",
                    "reason": f"Unsupported state: {resource_state}",
                }
    ...
    async def update_user_shortcuts(
        ...
        try:
            ...
            if new_files:
                ...
                for file_data in new_files:
                    try:
                        ...
                        if (
                            parsed.file_type.value == "audio"
                            or is_chart_accessible_by_user(parsed.key, user.instruments)
                        ):
                            accessible_files.append(file_data)
                    except Exception as e:
                        logger.warning(
                            f"Error parsing file {file_data.get('filename', 'unknown')}: {e}"
                        )
            ...
            if removed_files:
                shortcuts_deleted = await self._remove_user_shortcuts(
                    user, removed_files
                )
            ...
            user.user_folder.file_count = (
                user.user_folder.file_count + shortcuts_created - shortcuts_deleted
            )
    ...
    async def _get_users_for_sync(
        ...
        query = (
            select(User)
            .options(selectinload(User.user_folder), selectinload(User.band))
    ...
    async def _remove_user_shortcuts(
        self, user: User, removed_file_ids: List[str]
    ) -> int:
        """
        ...
        logger.info(
            f"Would remove shortcuts for {len(removed_file_ids)} files for user {user.id}"
        )
    ...
    async def get_sync_status(self, user_id: Optional[int] = None) -> Dict[str, Any]:
        ...
        try:
            if user_id:
                ...
                estimated_time = None
                if user.user_folder.sync_status in [
                    SyncStatus.PENDING,
    ...
    async def run_sync():
        ...
        try:
            await synchronizer.sync_source_to_user_folders(
                source_folder_id, user_ids=user_ids
            )
...
async def cleanup_stale_folders(
    synchronizer: FileSynchronizer, max_age_hours: int = 24
) -> Dict[str, Any]:
    """
